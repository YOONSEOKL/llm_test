...
openai.api_key = "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"
...
if os.path.isfile(file_path):
    print(f"{file_name} 파일이 존재하여 정상적으로 실행됩니다.")
    df = pd.read_csv(file_path)
    df['embedding'] = df['embedding'].apply(ast.literal_eval)

#그렇지 않다면 text열과 embedding열이 존재하는 df를 신규 생성해야 한다.
else:
    #정보원 내규 txt 파일명을 txt_files에 저장한다.
    folder_path = './data'
    txt_files = [file for file in os.listdir(folder_path) if file.endswith('.txt')]

    data = []

#txt_files로부터 정보원 내규 데이터를 로드하여 df를 신규 생성한다.
...
def create_prompt(df, query):
   ...
   system_role = f"""You are artificial intelligence language model named "기획실주혀니" that specializes in summarizing \
...
with st.form('form', clear_on_submit=True):
   user_input = st.text_input('우리원의 내부 규정을 물어보세요!', '', key='input')
